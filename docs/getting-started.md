...
`synth` is available on conda-forge:

```bash
mamba install -c conda-forge synth
```
...
To contribute to the development of `synth`, you can fork the repository and install the package in development mode.
...
```bash
git clone https://github.com/isce-framework/synth
```
